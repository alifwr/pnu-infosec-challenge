...
class ImageScraperBase:
    ...
    def download_image(
        self, url: str, query: str, source: str, extra_metadata: Optional[Dict] = None
    ) -> bool:
...
class GoogleImageScraper(ImageScraperBase):
    ...
    async def scrape(self, query: str, count: int = 100, headless=True) -> int:
...
class DuckDuckGoImageScraper(ImageScraperBase):
    ...
    async def scrape_with_browser(
        ...
        try:
            ...
            logger.error(f"Failed to load DuckDuckGo page: {str(e)}")
            logger.error(
                "This usually means: no internet connection, DNS resolution failed, or firewall is blocking DuckDuckGo"
            )
            return []
