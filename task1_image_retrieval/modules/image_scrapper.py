...
class ImageScraperBase:
    ...
    def download_image(self, url: str, query: str, source: str, extra_metadata: Dict = None) -> bool:
        ...
        try:
            ...
            if 'image' not in content_type.lower():
                logger.debug(f"Not an image (Content-Type: {content_type}): {url[:100]}")
                return False
            
            # Read content
            content = response.content
            if len(content) < 1024:  # Skip tiny images
            ...
            if not self._is_valid_image(img):
                logger.debug(f"Image too small ({img.size[0]}x{img.size[1]}): {url[:100]}")
                return False
...
class GoogleImageScraper(ImageScraperBase):
    ...
    async def scrape_with_browser(self, page: Page, query: str, count: int) -> List[str]:
        ...
        try:
            # Try to accept cookies (common dialogs)
            accept_button = page.locator('button:has-text("Accept all"), button:has-text("I agree"), button:has-text("Agree")')
        ...
        # Wait for images to load - use a more lenient approach
        ...
        while len(image_urls) < count and scroll_attempts < max_scrolls:
            ...
            # Extract image URLs - Google Images uses specific structure
            # Look for image elements within the results grid
            ...
            for img in images:
                try:
                    ...
                    # Priority: data-iurl (original), data-src, src
                    url = data_iurl or data_src or src
                    
                    # Filter out invalid URLs
                    if url and url.startswith('http') and 'base64' not in url:
                        # Skip Google's own UI images and tiny thumbnails
                        if not any(skip in url for skip in ['gstatic.com', 'google.com/images', 'encrypted-tbn']):
                            image_urls.add(url)
                            logger.debug(f"Added URL: {url[:100]}...")
                        
                        if len(image_urls) >= count * 2:  # Get extra for filtering
                            break
                except Exception as e:
                    continue
            
            # Also try to click on images to get high-res URLs
                    ...
                    for i, container in enumerate(containers[:5]):  # Check first few
                        try:
                            ...
                            if large_img:
                                ...
                                if large_src and large_src.startswith('http') and 'base64' not in large_src:
                                    if 'encrypted-tbn' not in large_src:
                                        image_urls.add(large_src)
                                        logger.debug(f"Added large image URL: {large_src[:100]}...")
                        except:
                            continue
                except:
                    pass
            ...
            if current_height == last_height:
                ...
                try:
                    more_button = await page.query_selector('input[value="Show more results"]')
                    if more_button:
...
class BingImageScraper(ImageScraperBase):
    ...
    async def scrape_with_browser(self, page: Page, query: str, count: int) -> List[str]:
        ...
        while len(image_urls) < count and scroll_attempts < max_scrolls:
            ...
            # Extract image URLs from thumbnail containers
            ...
            for img in images:
                try:
                    ...
                    if url and url.startswith('http'):
                        # DuckDuckGo uses proxy URLs, just use what we have
                        image_urls.add(url)
                except Exception as e:
                    continue
