...
from modules.image_scrapper import (
    GoogleImageScraper,
    BingImageScraper,
...
def load_config(config_path: str = "configs/dataset_collection.json") -> dict:
...
async def main():
    ...
    if config['sources'].get('google', True):
        ...
        scrapers.append(GoogleImageScraper(
            config['output_dir'],
    ...
    if config['sources'].get('bing', True):
        ...
        scrapers.append(BingImageScraper(
            config['output_dir'],
            min_width=config['min_width'],
            min_height=config['min_height']
        ))
    ...
    if config['sources'].get('duckduckgo', True):
        ...
        scrapers.append(DuckDuckGoImageScraper(
            config['output_dir'],
            min_width=config['min_width'],
            min_height=config['min_height']
        ))
    ...
    if not scrapers:
        logger.error("❌ No scrapers enabled! Please enable at least one source in config.")
        sys.exit(1)
    ...
    logger.info("  2. Analyze dataset: python analyze_dataset.py {}".format(config['output_dir']))
